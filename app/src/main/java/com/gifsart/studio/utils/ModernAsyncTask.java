...
/**
 * Copy of the required parts of {@link android.os.AsyncTask} from Android 3.0
 * that is needed to support AsyncTaskLoader. We use this rather than the one
 * from the platform because we rely on some subtle behavior of AsyncTask that
 * is not reliable on older platforms.
 * 
 * <p>
 * Note that for now this is not publicly available because it is not a complete
 * implementation, only sufficient for the needs of {@link android.content.AsyncTaskLoader}.
 */
public abstract class ModernAsyncTask<Params, Progress, Result> {
	...
	public enum Status {
		...
		RUNNING,
		/**
		 * Indicates that {@link android.os.AsyncTask#onPostExecute(Object)} has
		 * finished.
		 */
		FINISHED,
	}
